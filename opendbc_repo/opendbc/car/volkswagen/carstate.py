...
class CarState(CarStateBase):
  ...
  def update_meb(self, pt_cp, cam_cp, ext_cp) -> structs.CarState:
    ...
    # EA(Emergency Assist) 모듈이 활성 개입 단계(3~6)면 비치명 폴트 표시 (commaai/opendbc 동일).
    # 평시 2(STANDBY) 실차 확인. EA_01은 STOCK_EA_PRESENT일 때만 파싱됨.
    if self.CP.flags & VolkswagenFlags.STOCK_EA_PRESENT:
    ...
    # ACC 타입: 게이트웨이 하네스는 순정 레이더의 ACC_18에서 실측 (commaai/opendbc 동일,
    # RX CRC 실차 검증 MK1/MK2 각 3000 프레임 전수 통과). 카메라 하네스는 레이더 침묵이라 2 고정.
