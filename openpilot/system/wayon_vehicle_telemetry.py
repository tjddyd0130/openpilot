...
class TripRecorder:
  ...
  def finish(self, config: dict, device_id: str) -> bool:
    ...
    detail = f"({self.distance_m / 1000:.2f}km, {len(self.route)}pts)"
    print(f"Wayon telemetry: trip upload {status} {detail}", flush=True)
    return ok
...
def main() -> None:
  ...
  while True:
    ...
    if post_state(config, payload):
      ...
      soc = f"soc={vehicle.get('soc_percent')}%"
      odo = f"odo={vehicle.get('odometer_km')} charging={vehicle.get('charging')}"
      stale = f"stale={vehicle.get('stale', False)}"
      print(f"Wayon telemetry: uploaded {soc} {odo} {stale}", flush=True)
